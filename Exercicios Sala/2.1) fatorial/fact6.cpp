...
void remove_zeros_a_esquerda(vector<string>& v) 
{
    if(v.size() > 1) 
    {
        int cont=0;
...
void multiplicar_vetor_por_numero(vector<string>& resultado, const int& num) 
{
    int vai1 = 0;
    for (size_t r = 0; r < resultado.size() || vai1 > 0; r++) // multiplica todos os algarismos pelo numero
    {
        ...
        int produto = stoi(resultado[r]) * num + vai1; // menos um for se multiplicar numero direto ao inves de cada digito
        resultado[r] = to_string(produto % 10);
        vai1 = produto / 10;
    }
}
...
{
    vector<string> resultado; resultado.push_back("1"); // numero fica ao contrario porem mais simples de trabalhar
    ...
    string retorno = "";
    for(string& algarismo : resultado) 
        retorno += algarismo;
    return retorno;
}
